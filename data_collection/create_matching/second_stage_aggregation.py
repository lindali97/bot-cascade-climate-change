...
matching_data = pd.read_csv("/home/linda/data_collection_code/matching_user_data_collection_xr2019.csv")
matching_data['uid'] = [int(i) for i in matching_data.author_id]
    ...
    with open(file_name) as f:
        ...
        if file != "{\n":
            ...
            if flat_tweet.shape[0] != 0:
                ...
                all_tweet = flat_tweet.merge(matching_data[["uid", "interaction_at"]], how = "left", left_on = "author_id", right_on = "uid", copy = False)

                all_tweet['created_at_dt'] =  pd.to_datetime(all_tweet.created_at, infer_datetime_format=True)
                all_tweet['interaction_at_dt'] =  pd.to_datetime(all_tweet.interaction_at, infer_datetime_format=True)
